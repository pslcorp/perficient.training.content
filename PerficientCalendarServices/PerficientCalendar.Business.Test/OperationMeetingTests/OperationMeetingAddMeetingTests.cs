...
public class OperationMeetingAddMeetingTests
{
    ...
    public OperationMeetingAddMeetingTests()
    {
    ...
    public async Task Add200TestAsync()
    {
        ...
        MeetingRepositoryMock.Setup(x => x.Add(It.IsAny<MeetingDTO>())).ReturnsAsync(true);
        var response = await Operations.AddMeeting(new Meeting());
    ...
    public async Task Add400TestAsync()
    {
        OfficeRepositoryMock.Setup(x => x.GetById(It.IsAny<Guid>())).ReturnsAsync(new OfficeDTO());
        TypeEventRepositoryMock.Setup(x => x.GetById(It.IsAny<Guid>())).ReturnsAsync(new TypeEventDTO());
        MeetingRepositoryMock.Setup(x => x.Add(It.IsAny<MeetingDTO>())).ReturnsAsync(false);
        var response = await Operations.AddMeeting(new Meeting());
